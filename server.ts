...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    const sanitizedItems = items.map((item: any) => ({
      product_id: String(item.product_id || item.id || "1"),
      product_name: String(item.product_name || item.name || item.title || `Producto #${item.product_id || item.id || "1"}`),
      quantity: Math.max(1, Number(item.quantity || 1)),
      unit_price: Math.max(0.01, Number((parseFloat(item.unit_price || item.price || 0) || 0).toFixed(2)))
    }));
    ...
    let initPoint = null;
    const mpConfig = await db.getMercadoPagoConfig();
    const token = mpConfig.accessToken || mpSettings.accessToken || process.env.MERCADOPAGO_ACCESS_TOKEN;
    ...
    if (selectedPaymentMethod === "mercadopago") {
      if (token) {
        try {
          ...
          const mpItems = sanitizedItems.map((item: any) => {
            const rawPrice = Number(item.unit_price) || 1;
            // Apply 5% discount for Mercado Pago payments
            const discounted = Number((rawPrice * 0.95).toFixed(2));
            const finalUnitPrice = discounted > 0 ? discounted : 1;

            return {
              id: String(item.product_id || "1"),
              title: String(item.product_name || `Producto #${item.product_id || "1"}`),
              quantity: Math.max(1, Number(item.quantity || 1)),
...
// Admin Mercado Pago Configuration Endpoints (Persisted in Supabase & Local JSON database)
app.get("/api/admin/mercadopago/config", requireAdmin, async (req, res) => {
  try {
    const config = await db.getMercadoPagoConfig();
    return res.json({
      accessToken: config.accessToken || mpSettings.accessToken || process.env.MERCADOPAGO_ACCESS_TOKEN || "",
      publicKey: config.publicKey || mpSettings.publicKey || process.env.VITE_MERCADOPAGO_PUBLIC_KEY || "",
      sandbox: config.sandbox ?? false,
      configured: config.configured,
      updated_at: config.updated_at,
      storage: db.isSupabase ? "Supabase (PostgreSQL)" : "Local JSON Storage"
    });
  } catch (error: any) {
    ...
    return res.json({
      ...
      storage: "In-Memory / Environment Fallback"
    });
  }
});

app.post("/api/admin/mercadopago/config", requireAdmin, async (req, res) => {
  try {
    ...
    const saved = await db.saveMercadoPagoConfig({
      ...
      publicKey: publicKey !== undefined ? String(publicKey).trim() : undefined,
      sandbox: sandbox !== undefined ? Boolean(sandbox) : undefined
    });

    // Update in-memory fallback
    mpSettings.accessToken = saved.accessToken;
    mpSettings.publicKey = saved.publicKey;

    return res.json({
      success: true,
      message: `Configuración de Mercado Pago guardada y persistida exitosamente en ${db.isSupabase ? "Supabase" : "la base de datos"}.`,
    ...
    console.error("Error saving Mercado Pago configuration:", error);
    return res.status(500).json({ error: "Error al guardar y persistir la configuración de Mercado Pago" });
  }
});

// Safe public endpoint to get Mercado Pago Public Key for client-side brick/checkout
app.get("/api/mercadopago/public-key", async (req, res) => {
  try {
    const config = await db.getMercadoPagoConfig();
    const publicKey = config.publicKey || mpSettings.publicKey || process.env.VITE_MERCADOPAGO_PUBLIC_KEY || "";
    return res.json({
      publicKey,
      configured: config.configured
    });
